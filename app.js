/**
 * Express server application for managing accounts with user authentication.
 * 
 * This application provides endpoints for:
 * - user login to generate a JWT token.
 * - Creating accounts.
 ...
 * In a production environment, sensitive data (such as user credentials and jwtSecret)
...
/**
 * user credentials and JWT configuration.
 * 
 * userUsername: The hardcoded username for the user account.
 * userPassword: The hardcoded password for the user account.
...
app.post('/login', (req, res) => {
  ...
  if (username === userUsername && password === userPassword) {
    // Generate a JWT token with an user role claim and set its expiration time
    const token = jwt.sign({ username, role: 'user' }, jwtSecret, { expiresIn: tokenExpiry });
...
/**
 * Middleware to verify the JWT token for user access.
 * 
 ...
 * payload to req.user. If the token is missing or invalid, it returns a 403 Forbidden error.
 * 
 * Parameters:
